		...
		<div class="col-lg-9" style = "border-left:1px solid #ccc;">
			<div class="col-lg-12">
				<p><input type="text" id = "title" class="form-control" placeholder = "请填写标题" /></p>
				<textarea name="mdeditor" id = "mdeditor" rows="40"><?php echo $mdcontent; ?></textarea>
				<button class = "btn btn-success" id = "save"> 保 存</button>
				<a href="./tpre/<?php echo $fileid; ?>" class = "btn btn-info" style = "margin-left:12px;" target = "_blank">预 览</a> 
...
<script>
	...
	$(document).ready(function(){
		$("#save").click(function(){
			...
			$.post("./savedoc",{title:title,content:content,choose:choose},function(data,status){
				if(data == '200') {
					$("#msg").show();
					$("#msg").text('保存成功！');
					$("#msg").fadeOut(2000);
